    ...
    class ERE_Agent_Admin
    {
        ...
        public function save_agent_meta($post_id, $post)
        {
            ...
            if ($user_as_agent) {
                ...
                $agent_email = sanitize_email($_POST[ERE_METABOX_PREFIX . 'agent_email']);
        ...
        public function approve_agent()
        {
            ...
            if (!empty($approve_agent) && wp_verify_nonce($_wpnonce, 'approve_agent') && current_user_can('publish_post', $approve_agent)) {
                $listing_data = array(
                    'ID' => $approve_agent,
                    'post_status' => 'publish'
                );
        ...
        public function filter_restrict_manage_agent() {
            ...
            if ($typenow == $post_type) {
                ...
                wp_dropdown_categories(array(
                    'show_option_all' =>  sprintf(__('All %s','essential-real-estate', esc_html($info_taxonomy->label))),
                    'taxonomy'        => $taxonomy,
                    'name'            => $taxonomy,
                    'orderby'         => 'name',
                    'selected'        => $selected,
